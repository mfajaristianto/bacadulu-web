    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div data-bd-reveal="up">
            <div class="inline-flex items-center gap-2 text-orange-600">
                ...
                <span class="text-xs font-bold tracking-widest uppercase">Jejaring dan Afiliasi</span>
            </div>

            <h2 class="text-3xl sm:text-4xl font-extrabold text-slate-900 mt-2">Affiliated By</h2>
        ...
        <div class="bd-affiliate-wrap mt-10" data-bd-reveal="zoom">
            <div class="bd-affiliate-track">
                ...
                @foreach([0,1] as $copy)
                    ...
                    <div class="bd-affiliate-logo" {{ $copy?'aria-hidden=true':'' }}>
                        <img src="{{ asset('img/'.$logo['src']) }}" alt="{{ $copy?'':$logo['alt'] }}" title="{{ $logo['alt'] }}">
                    </div>
                    @endforeach
                @endforeach
...
.bd-affiliate-track{display:flex;width:max-content;gap:16px;animation:bdAffiliate 38s linear infinite;will-change:transform}
.bd-affiliate-wrap:hover .bd-affiliate-track{animation-play-state:paused}
.bd-affiliate-logo{width:180px;height:108px;flex:0 0 auto;display:flex;align-items:center;justify-content:center;padding:20px;border:1px solid #E2E8F0;border-radius:16px;background:#F8FAFC;transition:transform .3s ease,border-color .3s ease,box-shadow .3s ease}
...
@keyframes bdAffiliate{from{transform:translateX(0)}to{transform:translateX(calc(-50% - 8px))}}
@media(prefers-reduced-motion:reduce){.bd-affiliate-track{animation:none}}
</style>