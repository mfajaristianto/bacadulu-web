...
.detail-reveal{opacity:0;filter:blur(3px);translate:0 30px;transition:opacity .72s var(--ease),translate .72s var(--ease),filter .72s ease;transition-delay:var(--delay,0ms)}
.detail-reveal.detail-visible{opacity:1;filter:none;translate:0 0}
.cover-3d-wrapper{position:relative;display:flex;align-items:center;justify-content:center;padding:45px 20px;overflow:hidden;background:linear-gradient(135deg,#FBF9F5,#F1EDE4);border:1px solid #EAE7DF;border-radius:19px;box-shadow:0 18px 38px -15px rgba(27,36,83,.09);perspective:1200px;isolation:isolate}
...
@media(max-width:767px){.format-grid{grid-template-columns:1fr}.detail-main-card{padding:23px}.detail-info-grid{grid-template-columns:1fr 1fr}.description-text{font-size:13px}.current-price{font-size:21px}}
@media(prefers-reduced-motion:reduce){.detail-reveal{opacity:1!important;filter:none!important;translate:0!important;transition:none!important}.book3d-box,.cover-3d-wrapper::after{animation:none!important}.detail-format-card,.detail-info-box,.bibliography-card{transform:none!important}}
</style>
        ...
        <div class="lg:col-span-4 space-y-6 lg:sticky lg:top-24">
            <div class="cover-3d-wrapper detail-reveal">
                <div class="book3d-box" id="detailBook3d">
                    ...
                    <div class="detail-book-face detail-book-front" style="@if($book->cover)background-image:url('{{ asset('storage/'.$book->cover) }}');@else background:linear-gradient(135deg,#EF5843,#F7AA35);@endif">
                        @if(!$book->cover)
                            <div>
                                <div style="font-family:'Poppins',sans-serif;font-size:18px;font-weight:700;line-height:1.35;">{{ $book->title }}</div>
                                <div style="margin-top:7px;font-size:11px;opacity:.85;">{{ $book->author }}</div>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="bibliography-card detail-reveal">
                ...
                <div class="biblio-list">
                    <div><span class="biblio-label">Judul Buku</span><span class="biblio-value">{{ $book->title }}</span></div>
                    <div><span class="biblio-label">Penulis</span><span class="biblio-value">{{ $book->author }}</span></div>
                    <div><span class="biblio-label">Penerbit</span><span class="biblio-value">{{ $book->publisher ?: 'Baca Dulu' }}</span></div>
                    <div class="biblio-grid">
                        <div><span class="biblio-label">Tahun</span><span class="biblio-value">{{ $book->publish_year ?? '-' }}</span></div>
                        <div><span class="biblio-label">ISBN</span><span class="biblio-value">{{ $book->isbn ?? '-' }}</span></div>
                    </div>
                    <div class="biblio-grid">
                        <div><span class="biblio-label">Halaman</span><span class="biblio-value">{{ $book->pages ?? '-' }}</span></div>
                        <div><span class="biblio-label">Ukuran</span><span class="biblio-value">{{ $book->size ?? '-' }}</span></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-8">
            <div class="detail-main-card detail-reveal">
                ...
                <div class="detail-info-grid">
                    <div class="detail-info-box"><small>Halaman</small><strong>{{ $book->pages ?? '-' }}</strong></div>
                    <div class="detail-info-box"><small>Kategori</small><strong>{{ $book->category ?? 'Umum' }}</strong></div>
                </div>
                ...
                <div class="format-area">
                        ...
                        @if($book->has_print&&$book->print_price!==null)
                            <div class="detail-format-card print">
                                ...
                                @if($book->has_active_print_discount)
                                    <div class="old-price">IDR {{ number_format((float)$book->print_price,2,',','.') }}</div>
                                @else
                                    <div class="old-price" style="text-decoration:none">&nbsp;</div>
                                @endif

                                <div class="current-price">IDR {{ number_format((float)$book->effective_print_price,2,',','.') }}</div>

                                @if($book->has_active_print_discount)
                                    <span class="discount-badge">Diskon {{ number_format((float)$book->print_discount_percent,0) }}%</span>
                                @endif

                                <button type="button" class="add-format-button detail-add-cart"
                        ...
                        @if($book->has_ebook&&$book->ebook_price!==null)
                            <div class="detail-format-card ebook">
                                ...
                                @if($book->has_active_ebook_discount)
                                    <div class="old-price">IDR {{ number_format((float)$book->ebook_price,2,',','.') }}</div>
                                @else
                                    <div class="old-price" style="text-decoration:none">&nbsp;</div>
                                @endif

                                <div class="current-price">IDR {{ number_format((float)$book->effective_ebook_price,2,',','.') }}</div>

                                @if($book->has_active_ebook_discount)
                                    <span class="discount-badge">Diskon {{ number_format((float)$book->ebook_discount_percent,0) }}%</span>
                                @endif
                    ...
                    @if(!$book->has_print&&!$book->has_ebook)
                        <div class="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">Format buku belum tersedia.</div>
                    @endif
                ...
                <div class="description-section">
                    ...
                    <div class="description-text">{{ $book->description ?? 'Belum ada deskripsi.' }}</div>
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    reveals.forEach((el,index)=>el.style.setProperty('--delay',`${Math.min(index,5)*80}ms`));

    if(!reduceMotion){
        const observer=new IntersectionObserver(entries=>{
            entries.forEach(entry=>{
                if(entry.isIntersecting)entry.target.classList.add('detail-visible');
                else entry.target.classList.remove('detail-visible');
            });
        },{threshold:.08,rootMargin:'0px 0px -5% 0px'});
        reveals.forEach(el=>observer.observe(el));
    }else reveals.forEach(el=>el.classList.add('detail-visible'));
    ...
    if(!reduceMotion&&coverWrapper&&book3d){
        coverWrapper.addEventListener('pointermove',event=>{
            ...
            book3d.style.transform=`rotateX(${rx}deg) rotateY(${ry}deg) translateY(-5px)`;
        });
    ...
    function addToCart(button){
        ...
        if(feedbackText)feedbackText.textContent=`${product.format} berhasil ditambahkan ke keranjang.`;

        const span=button.querySelector('span');
        const original=span?.textContent??'Tambah ke Keranjang';
        if(span)span.textContent='✓ Ditambahkan';
