    ...
    <form action="{{ route('admin.books.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl border p-6 shadow-sm space-y-4">
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="number" name="original_price" placeholder="110000" class="w-full rounded border border-slate-300 p-2 text-sm">
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">Rating (Contoh: 4.9)</label>
                <input type="text" name="rating" placeholder="4.9" value="4.9" class="w-full rounded border border-slate-300 p-2 text-sm">
            </div>
        </div>
